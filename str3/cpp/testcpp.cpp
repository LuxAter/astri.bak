 ...
 *                        add switch for afspc compatibility and improved
 * operation changes : 14 may 08  david vallado fixes for linux suggested by
 * brian micek misc fixes noted by the community - manual
 * operation, formats, char lengths 14 aug 06  david
 * vallado update mfe for verification time steps, constants 20 jul
 * 05  david vallado fixes for paper, corrections from paul crawford
...
int main() {
  ...
  while (feof(infile) == 0) {
    ...
    if (feof(infile) == 0) {
      ...
      if (fabs(tsince) > 1.0e-8) tsince = tsince - deltamin;
        ...
        if (satrec.error == 0) {
          if ((typerun != 'v') && (typerun != 'c')) {
            ...
            fprintf(outfile,
                    ...
                    "%5i%3i%3i %2i:%2i:%9.6f\n",
                    tsince, ro[0], ro[1], ro[2], vo[0], vo[1], vo[2], year, mon,
                    day, hr, min, sec);
            //                            fprintf(outfile, " %16.8f %16.8f
            //                            %16.8f %16.8f %12.9f %12.9f %12.9f\n",
            ...
            fprintf(outfilee,
                    " %16.6f %16.8f %16.8f %16.8f %12.9f %12.9f %12.9f \n",
            ...
            rv2coe(ro, vo, mu, p, a, ecc, incl, node, argp, nu, m, arglat,
                   truelon, lonper);
            fprintf(outfile,
