...
IcosaSphere::IcosaSphere(float radius, unsigned sub)
    : radius_(radius), sub_(sub) {
  gen_vertices();
}

void IcosaSphere::draw() const {
  glBindVertexArray(VAO);
  glDrawElements(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, 0);
}

void IcosaSphere::add_instance() { instances.push_back(glm::mat4(1.0)); }
void IcosaSphere::set_color(int r, int g, int b) {
  red_ = r / 255.0;
  green_ = g / 255.0;
  blue_ = b / 255.0;
}
void IcosaSphere::set_color(float r, float g, float b) {
  red_ = r;
  green_ = g;
  blue_ = b;
}

std::vector<float> IcosaSphere::compute_vertices() {
...
void IcosaSphere::subdivide_vertices() {
...
void IcosaSphere::build_interleaved() {
  interleaved.clear();
...
void IcosaSphere::add_vertices(float x, float y, float z) {
  vertices.push_back(x);
  vertices.push_back(y);
  vertices.push_back(z);
}
void IcosaSphere::add_vertices(const float v1[3], const float v2[3],
                               const float v3[3]) {
...
void IcosaSphere::add_normals(float nx, float ny, float nz) {
  normals.push_back(nx);
  normals.push_back(ny);
  normals.push_back(nz);
}
void IcosaSphere::add_normals(const float n1[3], const float n2[3],
                              const float n3[3]) {
...
void IcosaSphere::add_indices(unsigned i1, unsigned i2, unsigned i3) {
  indices.push_back(i1);
  indices.push_back(i2);
  indices.push_back(i3);
}

void IcosaSphere::compute_face_normal(const float v1[3], const float v2[3],
...
void IcosaSphere::compute_half_vertex(const float v1[3], const float v2[3],
...
float IcosaSphere::compute_scale_for_length(const float v[3], float length) {
